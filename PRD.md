...
mini OpenClaw 的 Agent Skills 遵循 "Instruction-following" (指令遵循) 范式，而非传统的 "Function-calling" (函数调用) 范式。这意味着 Skills 本质上是教会 Agent 如何使用基础工具（如 Python/Terminal）去完成任务的说明书，而不是预先写好的 Python 函数。
